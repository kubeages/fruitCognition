...
```bash
docker build -f fruitAGNTCY/fruit_agents/fruit_cognition/docker/Dockerfile.auction-supervisor -t auction-supervisor .
```
...
#### Build the Docker Image
```bash
docker build -f fruitAGNTCY/fruit_agents/fruit_cognition/docker/Dockerfile.ui -t ui-server .
```
