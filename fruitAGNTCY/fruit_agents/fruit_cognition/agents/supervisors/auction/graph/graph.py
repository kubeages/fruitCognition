...
class ExchangeGraph:
    ...
    async def _supervisor_node(self, state: GraphState) -> dict:
        ...
        prompt = PromptTemplate(
            template=f"""You are a global fruit exchange agent connecting users to fruit farms: {farm_list}.
            ...
            - Respond with 'none of the above' if the message is unrelated to fruit 'inventory' or 'orders'
    ...
    async def _inventory_single_farm_node(self, state: GraphState) -> dict:
        ...
        try:
            ...
            prompt = PromptTemplate(
                template="""You are an inventory broker for a global fruit exchange company.
    ...
    async def _inventory_all_farms_node(self, state: GraphState) -> dict:
        ...
        try:
            ...
            final_content = f"Here is the current fruit yield inventory from the farms:\n\n{full_response.strip()}"
    ...
    async def _orders_node(self, state: GraphState) -> dict:
        ...
        prompt = PromptTemplate(
            template="""You are an orders broker for a global fruit exchange company.
            Your task is to handle user requests related to placing and checking orders with fruit farms.
