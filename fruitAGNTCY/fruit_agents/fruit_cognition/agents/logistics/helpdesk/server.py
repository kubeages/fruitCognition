...
factory = AgntcyFactory("fruit_cognition.logistics_helpdesk", enable_tracing=not OTEL_SDK_DISABLED)

logger = logging.getLogger("fruit_cognition.logistics.helpdesk.server")
...
async def liveness_probe(request):
    ...
    config = ClientConfig(
        slim_config=SlimTransportConfig(
            ...
            name="fruit_cognition/agents/helpdesk_agent",
