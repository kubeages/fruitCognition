...
logger = logging.getLogger("fruit_cognition.brazil_farm_agent.agent")

litellm_proxy_base_url = os.getenv("LITELLM_PROXY_BASE_URL")
litellm_proxy_api_key = os.getenv("LITELLM_PROXY_API_KEY")

# Brazil farm is LlamaIndex + LiteLLM. The proxy exposes OpenAI-style /v1/chat/completions
# and expects model=azure/gpt-4o in the body. If we don't force the client, LiteLLM sees
# "azure/gpt-4o" and uses the Azure provider, which builds an Azure-style URL and gets 404.
# Force custom_llm_provider="openai" so requests go to api_base/chat/completions like curl.
# Use a metadata-safe model name so LlamaIndex's context-size lookup doesn't raise.
if litellm_proxy_base_url and litellm_proxy_api_key:
    logger.info("Using LLM via LiteLLM proxy: %s", litellm_proxy_base_url)
    llm = LiteLLM(
        model=LLM_MODEL.removeprefix("azure/"),
...
def handle_inventory_tool(user_message: str) -> str:
    ...
    prompt = (
        "You are a helpful mango farm cultivation manager in Brazil who handles yield or inventory requests. "
        "Your job is to return a random yield estimate for the mango farm in Brazil. "
...
def classify_intent(user_message: str) -> IntentType:
    prompt = (
        "You are a mango farm manager in Brazil who delegates farm cultivation "
        ...
        "  product availability, or specific mango item details.\n"
...
async def main():
    ...
    messages = [
        "How much mango do we have in stock?",
    ...
    messages = [
        "Can you order me 100 lbs of mango?",
        "I want to place a new order for 50 kg of mango",
    ]
    for msg in messages:
        print(f"\nUser: {msg}")
        final_state = await agent.llama_index_invoke(msg)
