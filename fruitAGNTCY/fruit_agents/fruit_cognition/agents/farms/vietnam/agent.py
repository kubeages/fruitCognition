...
This module implements a multi-agent system for the Vietnam banana farm.
...
logger = logging.getLogger("fruit_cognition.vietnam_farm_agent.agent")
...
inventory_agent = Agent(
    ...
    description="Handles yield, stock, inventory, and product availability requests for the Vietnam banana farm. Delegate to this agent for questions about banana quantities, harvest estimates, or what's in stock.",
    instruction="""You are a helpful banana farm cultivation manager in Vietnam who handles yield or inventory requests.
Return a random yield estimate for the banana farm in Vietnam. Make sure the estimate is a reasonable value and in pounds.
...
orders_agent = Agent(
    name="orders_agent",
    model=LiteLlm(model=LLM_MODEL),
    description="Handles order-related queries for the Vietnam banana farm including checking order status, placing new orders, and modifying existing orders. Delegate to this agent for any order or purchase related questions.",
...
root_agent = Agent(
    ...
    model=LiteLlm(model=LLM_MODEL),
    description="The main coordinator agent for the Vietnam banana farm. Routes requests to specialized sub-agents.",
    instruction="""You are the main coordinator for the Vietnam banana farm, managing a team of specialized agents.
...
1. 'inventory_agent': Handles yield, stock, inventory, and product availability questions. Delegate to it for questions like "How much banana do we have?", "What's the yield estimate?", "What's in stock?"
2. 'orders_agent': Handles order-related queries including status checks, placing orders, and modifications. Delegate to it for questions like "Place an order", "Check order status", "I want to buy banana"
...
async def main():
    ...
    messages = [
        "How much banana do we have in stock?",
    ...
    messages = [
        "Can you order me 100 lbs of banana?",
        "I want to place a new order for 50 kg of banana",
    ]
    for msg in messages:
        print(f"\nUser: {msg}")
        final_state = await agent.google_adk_agent_invoke(msg)
