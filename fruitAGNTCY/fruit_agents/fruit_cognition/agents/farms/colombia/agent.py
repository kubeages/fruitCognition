...
logger = logging.getLogger("fruit_cognition.colombia_farm_agent.agent")

# Initialize a multi-protocol, multi-transport agntcy factory.
factory = AgntcyFactory("fruit_cognition.colombia_farm", enable_tracing=not OTEL_SDK_DISABLED)
...
class FarmAgent:
    ...
    def _supervisor_node(self, state: GraphState) -> dict:
        ...
        prompt = PromptTemplate(
            template="""You are a apple farm manager in Colombia who delegates farm cultivation and global sales. Based on the
            user's message, determine if it's related to 'inventory' or 'orders'.
            Respond with 'inventory' if the message is about checking yield, stock, product availability, or specific apple item details.
    ...
    async def _get_weather_forecast(self,state: GraphState) -> str:
        ...
        "fruit_cognition_weather_service" agent, lists available tools, and calls the "get_forecast" tool with a
        ...
        try:
            mcp_ctx = await factory.mcp().create_client(
                topic="fruit_cognition_weather_service",
    ...
    async def _inventory_node(self, state: GraphState) -> dict:
        ...
        prompt = PromptTemplate(
            template="""You are a helpful Colombian apple farm manager.
                You should estimate the seasonal apple yield after checking the weather given.
