# Adding Farm Agents (or Sub-Agents) to FruitCognition (Auction Supervisor Flow Only)

This document defines the **minimum configuration** and a **common contract** for adding new “farm” agents that can be supervised by the FruitCognition Auction Supervisor.
...
- **`name`**: a stable, human-readable name (e.g., `Kenya Fruit Farm`).
...
- `"How much fruit does the Kenya farm have?"`
...
```text
You are a fruit farm manager in {FARM_COUNTRY} who delegates farm cultivation and global sales.

Based on the user's message, determine whether it is related to 'inventory' or 'orders'.
- Respond with 'inventory' if the message is about checking yield, stock, product availability, or specific fruit item details.
...
Inventory behavior:
- Return a random yield estimate for the fruit farm in {FARM_COUNTRY}.
...
- Support two request types:
  - Inventory/yield (messages like `"How much fruit does the Kenya farm have?"` or `"What yield do the farms have?"`)
