...
strategy:
  matrix:
    image:
      - name: new-component
        dockerfile: fruitAGNTCY/fruit_agents/.../docker/Dockerfile.new
```

Ensure Dockerfile path is correct and name unique under ghcr.io/<org>/fruit-agntcy/.

## helm-push

Packages each chart listed under matrix.chart. On push to main charts are pushed to ghcr.io/<org>/fruit_agntcy/helm as OCI artifacts; PRs only lint + package.

To add a chart:

```yaml
- name: my-service
  path: fruitAGNTCY/fruit_agents/.../deployment/helm/my-service
